...
TIER_LIMITS = {
    ...
    SubscriptionTier.ENTERPRISE: {"max_file_mb": 2048, "jobs_per_month": 999999},
}
...
async def upload_file(
    ...
    if ext not in ALLOWED_EXTENSIONS:
        raise HTTPException(status_code=400, detail=f"File type '{ext}' not supported. Use: csv, tsv, xlsx, xls")

    limits = TIER_LIMITS[current_user.subscription_tier]
    if current_user.jobs_used_this_month >= limits["jobs_per_month"]:
        raise HTTPException(status_code=429, detail="Monthly job limit reached. Upgrade your plan.")
    ...
    async with aiofiles.open(upload_path, "wb") as out:
        while chunk := await file.read(1024 * 1024):  # stream 1 MB at a time
            ...
            if file_size > max_bytes:
                ...
                raise HTTPException(status_code=413, detail=f"File exceeds {limits['max_file_mb']} MB limit for your plan")
